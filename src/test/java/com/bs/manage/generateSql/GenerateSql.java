package com.bs.manage.generateSql;
...
public class GenerateSql {
    ...
    /*** sql脚本所在的绝对路径*/
    private static final String FILE_NAME = "D:\\project\\bs-manage\\src\\test\\java\\com\\bs\\manage\\generateSql\\createTable.sql";
    /*** 是否需要下划线转驼峰*/
    ...
    /*** 假删除时间的名字*/
    private static final String DELETE_TIME_NAME = "deleted_at";
...
//        }
        for (ColumnAndType columnAndType : columnAndTypeList) {
            ...
            sb.append("\t/**\n\t * ");
            if (StringUtils.hasText(columnAndType.getComment())){
                sb.append(columnAndType.getComment());
            }
            sb.append("\n\t */\n");
            if (idName.equals(columnAndType.getColumn())){
                sb.append("\t@TableId(value = \"").append(idName).append("\", type = IdType.AUTO)\n");
            }
            sb.append("\tprivate ");
//            int typeLength = "LocalDateTime".length();
            if ("char".equals(columnAndType.getType()) || "varchar".equals(columnAndType.getType())
                    || "text".equals(columnAndType.getType()) || "json".equals(columnAndType.getType())) {
                sb.append("String");
                columnAndType.setType("String");
//                typeLength = typeLength - "String".length();
            } else if ("bigint".equals(columnAndType.getType())) {
                sb.append("Long");
                columnAndType.setType("Long");
//                typeLength = typeLength - "Long".length();
            } else if ("tinyint".equals(columnAndType.getType()) || "int".equals(columnAndType.getType())) {
                sb.append("Integer");
                columnAndType.setType("Integer");
//                typeLength = typeLength - "Integer".length();
            } else if ("float".equals(columnAndType.getType())) {
                sb.append("Float");
                columnAndType.setType("Float");
//                typeLength = typeLength - "Float".length();
            } else if ("double".equals(columnAndType.getType())) {
                sb.append("Double");
                columnAndType.setType("Double");
//                typeLength = typeLength - "Double".length();
            } else if ("decimal".equals(columnAndType.getType())) {
                sb.append("BigDecimal");
                columnAndType.setType("BigDecimal");
//                typeLength = typeLength - "BigDecimal".length();
                ...
                columnAndType.setType("LocalDateTime");
...
//            sb.append("//").append(columnAndType.getComment() == null ? "" : columnAndType.getComment()).append("\n");
    ...
    private static void setExcelColumn() {
        List<ColumnAndType> columnAndTypeList = new ArrayList<>(columnAndTypes);
        columnAndTypeList.removeIf(next -> beanIgnore.contains(next.getColumn()));
        String beanName = hump(tableName);
        String bean = toUpperFirst(beanName);
        StringBuilder sb = new StringBuilder();
        sb.append("List<").append(bean).append("> ").append(beanName).append("List = new ArrayList<>(totalRow - 1);\n");
        sb.append("for (int i = 1; i < totalRow + 1; i++) {\n");
        sb.append("\tRow row = sheet.getRow(i);\n\t");
        sb.append(bean).append(" ").append(beanName).append(" = new ").append(bean).append("();\n");
        for (int i = 0; i < columnAndTypeList.size(); i++) {
            ColumnAndType columnAndType = columnAndTypeList.get(i);
            sb.append("\t").append(beanName).append(".set").append(toUpperFirst(columnAndType.getColumn()));
            if ("String".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getString(row.getCell(").append(i).append(")));");
            } else if ("Long".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getLong(row.getCell(").append(i).append(")));");
            } else if ("Integer".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getInteger(row.getCell(").append(i).append(")));");
            } else if ("Float".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getFloat(row.getCell(").append(i).append(")));");
            } else if ("Double".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getDouble(row.getCell(").append(i).append(")));");
            } else if ("BigDecimal".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getBigDecimal(row.getCell(").append(i).append(")));");
            } else if ("LocalDateTime".equals(columnAndType.getType())) {
                sb.append("(ExcelUtils.getString(row.getCell(").append(i).append(")) == null ?\n" +
                        "                    null : LocalDateTime.parse(ExcelUtils.getString(row.getCell(").append(i).append(")), DateUtil.DATE_TIME_FORMATTER));");
            }
            sb.append("\n");
        }
        sb.append("\t").append(beanName).append("List.add(").append(beanName).append(");\n}\n");
        sb.append(beanName).append("Service.insertBatch(").append(beanName).append("List);\n");
        System.out.println(sb);
    }
    ...
    private static List<ColumnAndType> getColumnAndTypes() throws IOException {
        ...
        Pattern columnPattern = Pattern.compile("(?<=`)(\\w+)` +(int|tinyint|bigint|decimal|datetime|timestamp|float|double|char|varchar|text|json)");
        ...
        Pattern commentPattern = Pattern.compile("(?<=`)(\\w+)` +(int|tinyint|bigint|decimal|datetime|timestamp|float|double|char|varchar|text|json).*?COMMENT +'(.+)'");
