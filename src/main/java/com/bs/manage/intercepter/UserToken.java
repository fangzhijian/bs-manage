package com.bs.manage.intercepter;
...
public class UserToken {
    ...
    public void refreshCache(User user) {
        User cache = this.getCache(user.getApi_token());
        if (cache != null) {
            redisTemplate.opsForValue().set(String.format(RedisConstants.USER_CACHE, user.getApi_token()), user, maxLoginSeconds, TimeUnit.SECONDS);
        }
    }
